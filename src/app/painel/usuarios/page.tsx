...
import { ArrowLeft, Check, ChevronLeft, ChevronRight, KeyRound, LogOut, Pencil, Plus, Search, ShieldCheck, Trash2, X } from "lucide-react";
...
type RecuperacaoSenha = { id: string; usuarioId: string; nome: string; cpf: string; email: string; solicitadoEm: string };
...
export default function PaginaGestaoUsuarios() {
  ...
  const carregar = useCallback(async () => {
    try {
      const [listaUsuarios, listaRecuperacoes] = await Promise.all([
        ...
        solicitarApi<RecuperacaoSenha[]>("/api/usuarios/recuperacoes-senha", { autenticado: true }),
      ]);
  ...
  async function ativarNovaSenha(id: string) {
    definirErro(""); definirMensagem("");
    try { const resultado = await solicitarApi<{ mensagem: string }>(`/api/usuarios/recuperacoes-senha/${id}/ativar`, { method: "POST", autenticado: true }); definirMensagem(resultado.mensagem); await carregar(); }
    catch (falha) { definirErro(falha instanceof Error ? falha.message : "Não foi possível ativar a nova senha."); }
  }
  async function rejeitarRecuperacao(id: string) {
    definirErro(""); definirMensagem("");
    try { await solicitarApi<void>(`/api/usuarios/recuperacoes-senha/${id}`, { method: "DELETE", autenticado: true }); definirMensagem("Solicitação de recuperação rejeitada."); await carregar(); }
    catch (falha) { definirErro(falha instanceof Error ? falha.message : "Não foi possível rejeitar a solicitação."); }
  }
  ...
  return <main className="min-h-screen bg-fundo text-texto-principal">
    ...
    <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
      <section className="mb-8" aria-labelledby="recuperacoes-title">
        <div className="flex items-center gap-3"><div className="grid size-11 place-items-center bg-primaria/15 text-secundaria"><KeyRound size={22} /></div><div><p className="text-xs font-bold tracking-[0.16em] text-secundaria">SEGURANÇA</p><h2 id="recuperacoes-title" className="mt-1 text-xl font-semibold text-texto-principal">Recuperações de senha pendentes</h2></div></div>
        <div className="mt-4 space-y-3">{recuperacoes.length === 0 ? <p className="border border-borda bg-superficie p-5 text-sm text-texto-secundario">Nenhuma solicitação pendente.</p> : recuperacoes.map((recuperacao) => <article key={recuperacao.id} className="flex flex-col gap-4 border border-borda bg-superficie p-5 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-semibold text-texto-principal">{recuperacao.nome}</h3><p className="mt-1 text-sm text-texto-secundario">{formatarCpf(recuperacao.cpf)} · {recuperacao.email}</p><p className="mt-1 text-xs text-texto-secundario">Solicitada em {new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short" }).format(new Date(recuperacao.solicitadoEm))}</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => void rejeitarRecuperacao(recuperacao.id)} className="inline-flex items-center gap-2 border border-erro px-3 py-2 text-sm font-semibold text-erro hover:bg-erro/10"><X size={17} />Rejeitar</button><button type="button" onClick={() => void ativarNovaSenha(recuperacao.id)} className="inline-flex items-center gap-2 bg-primaria px-3 py-2 text-sm font-bold text-primaria-texto hover:bg-primaria-hover"><ShieldCheck size={17} />Ativar nova senha</button></div></article>)}</div>
      </section>
